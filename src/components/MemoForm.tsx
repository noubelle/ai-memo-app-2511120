...
export default function MemoForm({
  ...
  const handleGenerateTags = async () => {
    if (!formData.content.trim()) {
      setTagError('태그를 생성하려면 메모 내용이 필요합니다.')
      return
    }
    ...
    try {
      ...
      if (uniqueNewTags.length > 0) {
        ...
        setTagError('새로운 태그를 생성할 수 없습니다. (이미 존재하는 태그입니다)')
      }
    } catch (error) {
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="inline-flex items-center px-3 py-1 text-xs font-medium text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isGeneratingTags ? (
                    <>
                      <svg
              ...
              {/* 에러 메시지 */}
              {tagError && (
                <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded-lg flex items-start justify-between">
                  <p className="text-xs text-red-800 flex-1">{tagError}</p>
                  <button
                    type="button"
